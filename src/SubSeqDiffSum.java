...
public class SubSeqDiffSum {
    ...
    static int solve(List<Integer> A) {
        ...
        for(int i =0; i<n; i++){
            long x = A.get(i)%mod;
            max = (max%mod + (x * Math.pow(2,i)%mod))%mod;
            min = (min%mod + (x * Math.pow(2,(n-1-i))%mod))%mod;
        }
